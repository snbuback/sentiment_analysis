...
cat negativo-texto-emoticon.txt | tr -d ":\\-(" | tr -d ":(" |  tr -d ":\\-)" | tr -d ":)" | sort -R > negativo-sem-emoticon.txt

# Separa em 500 para treino e o restante para o teste
cat negativo-sem-emoticon.txt | head --lines=500 > negativo-treino.txt
...
# Retira os emoticons e embaralha o texto
cat positivo-texto-emoticon.txt | tr -d ":\\-(" | tr -d ":(" |  tr -d ":\\-)" | tr -d ":)" | sort -R > positivo-sem-emoticon.txt

# Separa em 500 para treino e o restante para o teste
cat positivo-sem-emoticon.txt | head --lines=500 > positivo-treino.txt
